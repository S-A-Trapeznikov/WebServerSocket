...
public class AddBook implements Command {
	...
	public Response execute(Request rq,Response rp) {
		...
		if ((rq.getContentType().toLowerCase().equals("application/json"))||(rq.getContentType().trim().startsWith("application/json"))){
		...
		boolean status = ls.getLibraryService().create(rq.getUri(),book);
		if (status){
